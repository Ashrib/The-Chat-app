    ...
    <div className="main">
      <div className="chat-main">
        <div className="chat-nav">
          <img src={state.user.profileImage} className='prf-img' alt="Profile Photo" height="40" width="40" />
          <Dropdown>
              <Dropdown.Toggle className="dropToggle" id="dropdown-button-dark-example1" variant="secondary">
              </Dropdown.Toggle>
        ...
        {(users?.length) ?
                  users?.map((eachUser, index) => {
                    ...
                    return <div className='usersList' key={index}>
                            <Link to={`/chat/${eachUser._id}`}>
                              <div className="user">
                                <div>
                                  <img className="prf-img" src={(!eachUser.profileImg)?"https://img.icons8.com/material-rounded/256/user.png":eachUser?.profileImg} alt="users profile" height="45" width="45" />
                                  <p>{eachUser?.firstName} {eachUser?.lastName}</p>
                                </div>
                                {(eachUser.isOnline === true)?
                                  <span style={{color:"green",fontSize:'0.8em'}}>active</span>:
                                  null
                                 }
